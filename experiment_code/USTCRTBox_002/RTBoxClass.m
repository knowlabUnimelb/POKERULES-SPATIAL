classdef RTBoxClass < handle
  ...
  methods (Hidden, Static)
    ...
    function out = instances(cmd, val)
      ...
      elseif strcmp(cmd, 'closeAll') % close all serial ports
        for i = 1:numel(boxes), close(boxes{i}); end
      end
    end
  end
  ...
  methods
    ...
    function varargout = TTL(obj, eCode)
      ...
      if ub>0.003, RTBoxWarn('USBoverload', ub); end
    end
    ...
    function clear(obj, nSyncTrial)
      ...
      %
      % The optional input nSyncTrial, default 20, is the number of trials to
  ...
  methods (Hidden)
    function t3 = syncClocks(obj, nr, enableInd)
      ...
      for i = 1:nr
        ...
        t(i,3) = bytes2secs(b7(2:7)', obj.p.clkRatio);
      end
