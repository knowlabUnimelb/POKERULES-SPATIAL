...
% 
% The optional second input nSyncTrial, default 20, is the number of trials to
...
% 
% Optionally, this command returns a 4-element row vector: the first is the time
...
% difference; and the last one is the time difference from another
% synchronization method (selecting a trial by min(tpost-tpre), and using its
% mean as tsend).
% 
...
switch cmd
        ...
        while 1
            if IOPort('BytesAvailable', s) >= 7
                if nargout
                    ...
                    info(id).sync = syncClocks(info(id), 10); % new sync
                    t = t + info(id).sync(1);
                end
                break;
            end
    ...
    case read % 4 trigger events, plus 'secs' 'boxsecs'
        ...
        if cmdInd==1 % secs: convert into computer time
            if timing(end)-info(id).sync(2) > 9 % sync done too long before?
                ...
                info(id).sync = syncClocks(info(id), 20, 1:2); % update sync
    ...
    case 'buttonnames' % set or query button names
        ...
        if ~isempty(intersect(newNames, info(id).events(9:end)))
    ...
    case 'clockratio' % measure clock ratio computer/box
        ...
        info(id).sync = syncClocks(info(id), 10, 1:2); % use new ratio
    case 'ttlwidth'
    ...
    case 'reset'
        ...
        info(id).sync = syncClocks(info(id), 10, 1:2);
    ...
    case 'info'
        ...
        elseif IsLinux
            ...
            os = regexp(os, 'Description:\s*(.*)', 'tokens', 'once', 'dotexceptnewline');
        elseif IsOSX
            [~, os] = system('system_profiler SPSoftwareDataType 2>&1');
            os = regexp(os, 'Kernel Version:\s*(.*)', 'tokens', 'once', 'dotexceptnewline');
        end
        ptbV = IOPort('Version');
...
function t4 = syncClocks(info, nr, enableInd)
s = info.handle;
v = info.version;
...
for i = 1:nr
    for iTry = 1:4
        ...
        b7 = IOPort('Read', s, 1, 7);
        if numel(b7)==7 && b7(1)==89, break; end
        purgeRTBox(s);
        if iTry==4, RTBoxError('notRespond'); end
    end
    t(i,3) = bytes2secs(b7(2:7)', info);
end
...
tdiff_ub = min(t(:,2)-t(:,3))-tdiff; % earliest tpost - lastest tpre
tbox = t(i,3); % tbox when diff measured
[~, i] = min(t(:,2)-t(:,1)); % find minwin index
method3_1 = mean(t(i, 1:2)) - t(i,3) - tdiff; % diff between methods 3 and 1
t4 = [tdiff+8.68e-05 tbox twin method3_1]; % tdiff, its tbox and ub's
...
while 1
    ...
    str = [str ln(2:end) cr]; %#ok
end

% detect topic line before formating the str: try each in order
while 1
    if isempty(strfind(str, [cr '  '])), break; end %#ok<*STREMP>
    str = strrep(str, [cr '  '], [cr ' ']);
end
...
% do some formating for reliable syntax detection (order is important):
...
str = strrep(str, cr, cr1); % add one space after cr
...
% if we reach here, no subcmd found in help text
