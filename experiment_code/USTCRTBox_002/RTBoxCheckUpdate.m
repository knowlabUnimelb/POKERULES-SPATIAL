...
mfile = 'RTBox';
myFileDate = reviseDate(mfile); % yymmdd
if nargout, varargout{1} = myFileDate; return; end
myFileDate = datenum(myFileDate, 'yymmdd');
...
try
    ...
    [~, v] = RTBoxPorts(1);
    if isempty(v), error('No RTBox hardware detected.');
    elseif numel(v)>1, v = v(1);
    end
    i = floor(vn)==floor(v);
    vs = vs{i};
    doFirmware = vn(i)>v;
